...
export async function enableBlobStorage(dir) {
  return perf.measureAsync('cache.enableBlobStorage', transport.command('cache_enable_blob_storage', { dir }));
}

/** 检查 Blob 缓存是否已启用 */
export async function isBlobStorageEnabled() {
  return perf.measureAsync('cache.blobStorageEnabled', transport.command('cache_blob_storage_enabled'));
}
...
export async function cacheSetBlob(key, data, ttl) {
  const bytes = data instanceof ArrayBuffer
    ...
    : data instanceof Uint8Array
      ? Array.from(data)
      : data;
  return perf.measureAsync('cache.setBlob', transport.command('cache_set_blob', { key, data: bytes, ttl }), { key });
}
...
export async function cacheGetBlob(key) {
  const result = await perf.measureAsync('cache.getBlob', transport.command('cache_get_blob', { key }), { key });
...
export async function cacheRemoveBlob(key) {
  return perf.measureAsync('cache.removeBlob', transport.command('cache_remove_blob', { key }), { key });
}

/** 检查 Blob 缓存项是否存在且未过期 */
export async function cacheHasBlob(key) {
  return perf.measureAsync('cache.hasBlob', transport.command('cache_has_blob', { key }), { key });
}

/** 获取所有未过期的 Blob 缓存 key */
export async function cacheBlobKeys() {
  return perf.measureAsync('cache.blobKeys', transport.command('cache_blob_keys'));
}

/** 清空所有 Blob 缓存（含磁盘文件） */
export async function cacheClearBlobs() {
  return perf.measureAsync('cache.clearBlobs', transport.command('cache_clear_blobs'));
}

/** 清理所有已过期的 Blob 缓存条目，返回清理数量 */
export async function cacheClearExpiredBlobs() {
  return perf.measureAsync('cache.clearExpiredBlobs', transport.command('cache_clear_expired_blobs'));
}
